...
const VehiclePanel = ({vehiclePannel,setVehiclePannel,setConfirmRidePannel,fare,setVehicleType}) => {
  const vehicles = [
    {
      name: "CaberGO",
      capacity: 4,
    ...
    >
      ...
      {vehicles.map((vehicle, index) => (
        ...
        >
          {/* Vehicle Image */}
          <img
            className="h-24 w-24 object-contain rounded-md mix-blend-multiply"
            src={vehicle.img}
            alt={vehicle.name}
          />
          ...
          <h2 className="text-lg font-semibold text-green-600">
            ₹{fare?.[vehicle.type] ? fare[vehicle.type].toFixed(2) : "N/A"}
          </h2>
        </div>
      ))}
    </motion.div>
