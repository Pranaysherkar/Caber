...
const RidePopUp = ({ ridePopUp, setRidePopUp, setConfirmRidePopUp }) => {
  return (
    <motion.div
      initial={{ y: "100%" }}
      animate={{ y: ridePopUp ? "0%" : "100%" }}
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-100 rounded-lg shadow-md">
        <h3 className="text-xl font-bold">Incoming Ride Request</h3>
      </div>
      ...
      <div className="flex flex-col items-center gap-5 mt-4">
        ...
        <div className="flex w-full gap-4">
          <button onClick={()=>setConfirmRidePopUp(true)} className="w-1/2 bg-green-500 text-white font-semibold py-3 rounded-lg shadow-md hover:bg-green-600 transition-all duration-300">
            Accept
          </button>
          <button onClick={() => setRidePopUp(false)} className="w-1/2 bg-red-500 text-white font-semibold py-3 rounded-lg shadow-md hover:bg-red-600 transition-all duration-300">
            Reject
          </button>
        </div>
      </div>
